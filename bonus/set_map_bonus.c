...
/*   Updated: 2023/04/19 17:46:58 by crigonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_point(t_coords *pt1, t_coords *pt2, t_fdf *fdf)
{
	...
	if (fdf->width > 80)
		pt1->z = (pt1->z * fdf->scale / 7);
	else if (fdf->width <= 21)
		pt1->z = (pt1->z * fdf->scale / 70);
	else
		pt1->z = (pt1->z * fdf->scale / 20);
	pt2->x = (pt2->x * fdf->scale);
	pt2->y = (pt2->y * fdf->scale);
	if (fdf->width > 80)
		pt2->z = (pt2->z * fdf->scale / 7);
	else if (fdf->width <= 21)
		pt2->z = (pt2->z * fdf->scale / 70);
	else
		pt2->z = (pt2->z * fdf->scale / 20);
	rotation(pt1, fdf);
	rotation(pt2, fdf);
	projection(pt1, fdf);
	projection(pt2, fdf);
